...
namespace AsyncEnum35Sample
{
    ...
    internal sealed class AsyncFileWriter
    {
        ...
        public IAsyncResult BeginWriteAllBytes(string path, byte[] bytes, AsyncCallback callback, object state)
        {
            return new WriteAllBytesAsyncOperation(path, bytes, this.bufferSize).Start(callback, state);
        }

        public void EndWriteAllBytes(IAsyncResult result)
        {
            WriteAllBytesAsyncOperation.End(result);
        }
