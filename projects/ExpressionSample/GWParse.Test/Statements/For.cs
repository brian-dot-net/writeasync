...
namespace GWParse.Test.Statements
{
    ...
    {
        [InlineData("FOR I=1 TO 10", "For(NumV(I), NumL(1), NumL(10))")]
        [InlineData("FOR A1=B1 TO C1", "For(NumV(A1), NumV(B1), NumV(C1))")]
        [InlineData("FOR A1=B(0) TO C(D)", "For(NumV(A1), NumA(B, NumL(0)), NumA(C, NumV(D)))")]
        [Theory]
